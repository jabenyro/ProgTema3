...
library(tidyverse)
library(plotly)
owid_covid_data <- read_csv(url("https://covid.ourworldindata.org/data/owid-covid-data.csv"))

#seleccionamos los paises que nos interesan:
countries <- c("United States", "Italy", "France", "Germany", "Canada", "Japan", "United Kingdom", "Spain")

graf <- owid_covid_data %>% filter(location %in% countries)
spain <- owid_covid_data %>% filter(location == "Spain")

#creamos el grafico:
grafico1 <- ggplot(data = graf, aes(x = date, y = new_deaths_smoothed_per_million, color = location)) + geom_line(size = 0.6) + geom_line(data = spain, size = 1, colour = "black") + labs(title = "Nuevas muertes diarias confirmadas por COVID-19 por millón de personas", subtitle = "En el siguiente gráfico se puede observar la comparacion de como ha afectado el Covid-19 entre 
distintos paises.", caption = "Fuente: Visualiza Grupo K, Universidad de Valencia", x ="" , y="", color =  "Países") + theme_classic() + theme(panel.grid.major.y = element_line(color = "grey",size = 0.1,linetype = 2))
grafico1
